...
export class AppComponent {
  ...
  interimInvoice: string[] = [];
  halfStoreArray: string[] = []
  storeArrayLength: string[] = []
  num: number = 0;
  numPvs: number = 0;
  secondNum: number = 0;
  secondNumPvs: number = 0;
  regexNumber = /[0-9\.]/;
  regexOperator = /[+*\/\-]/;
  regexMulDiv = /[*\/]/;
  regexPVS = /[\+||-]/
  ...
  result() {
    ...
    if (this.error === false) {
      ...
      this.pointbeforeline();
      console.log(this.interimInvoice)
  ...
  pointbeforeline(): void {
    for (var i = 0; i <= this.storeArray.length; i++) {
      if (this.regexMulDiv.test(this.storeArray[i]) === true) {
        this.interimInvoice = this.storeArray.splice(i - 1, i)

        this.numPvs = parseFloat(this.interimInvoice[0])
        this.secondNumPvs = parseFloat(this.interimInvoice[2])
        if (this.interimInvoice[1] === '*') {
          this.numPvs = this.numPvs * this.secondNumPvs
        }
        if (this.interimInvoice[1] === '/') {
          this.numPvs = this.numPvs / this.secondNumPvs
        }
        this.sumPvs = this.numPvs.toString();
